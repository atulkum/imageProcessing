...
// You will need to make corresponding changes to accommodate images of different sizes and types
	void LPF(unsigned char inImg[256][256], unsigned char outImg[256][256], int type){
		...
		/*OutputImagedata[0][0] = OutputImagedata[1][1];
		OutputImagedata[0][Size - 1] = OutputImagedata[1][Size - 2];
		OutputImagedata[Size - 1][0] = OutputImagedata[Size - 2][1];
		OutputImagedata[Size - 1][Size - 1] = OutputImagedata[Size - 2][Size - 2];

		for(int i=0; i<Size - 1; i++){
			OutputImagedata[i][0] = OutputImagedata[i][1];
			OutputImagedata[i][Size -2] = OutputImagedata[i][Size - 3];
		}
		for(int j=0; j<Size - 1; j++){
			OutputImagedata[0][j] = OutputImagedata[1][j];
			OutputImagedata[Size - 2][j] = OutputImagedata[Size - 3][j];
		}*/
	}
